...
class SpiderMain(object):
    def __init__(self, tableId, tableView):
        ...
        self.JSESSIONID = ''
        self.meta = ''
        self.url = 'http://www.nmpa.gov.cn/WS04/CL2170/'
    ...
    def getCookie(self):
        rsq = requests.get(self.url)
        rsq.close()
        # print(rsq.cookies)
        # 第一次请求得到假的f80s,f80t,和metacontent
        self.F80S = rsq.cookies['FSSBBIl1UgzbN7N80S']
        self.F80T = rsq.cookies['FSSBBIl1UgzbN7N80T']
        rsqHtml = etree.HTML(rsq.text)
        self.meta = rsqHtml.xpath('//*[@id="9DhefwqGPrzGxEp9hPaoag"]/@content')[0]
        self.F80T_true = ecjs.call("getcookie", self.meta, self.F80T)
        self.cookies['FSSBBIl1UgzbN7N80S'] = self.F80S
        self.cookies['FSSBBIl1UgzbN7N80T'] = self.F80T_true
        return self.cookies
        # rsq = requests.get(self.url, cookies=self.cookies)
        # print(rsq.cookies)
        # print(rsq.status_code)
        # print(rsq.content.decode('gbk'))
